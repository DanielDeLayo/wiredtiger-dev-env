...
WTPERF_CONFIG="$WT_DIR/bench/wtperf/runners/ycsb-50read50update.wtperf"
RESULTS_DIR="$WT_DIR/build/verify_results"

CACHE_SIZES=("2M" "5M" "10M" "15M" "20M")
RUN_TIME=10
ICOUNT=100000
...
for size in "${CACHE_SIZES[@]}"; do
    ...
    $WTPERF_CMD -O "$WTPERF_CONFIG" -o conn_config="\"cache_size=$size,log=(enabled=false)\"" -o run_time=$RUN_TIME -o icount=$ICOUNT -o warmup=2 > wtperf_out.log 2>&1 || true
